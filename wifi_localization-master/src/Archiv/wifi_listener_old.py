...
def inject(data):
	...
	print 'Database @',injectLoc,'contains ',len(database[injectLoc].keys()),'different hotspot entries.'
	
#serialize the data for storage
def clean():
	print 'Database contains ',len(database.keys()),'different Positions.'
	
	for position in database.keys():
		print 'Database @',position,'contains ',len(database[position].keys()),'different hotspot entries.'
